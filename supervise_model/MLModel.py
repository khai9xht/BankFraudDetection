...
from sklearn.model_selection import train_test_split, KFold, \
            StratifiedKFold, cross_val_score, GridSearchCV, ShuffleSplit
from sklearn.metrics import precision_score, recall_score, f1_score, \
                    roc_auc_score, accuracy_score, classification_report
import numpy as np
import pandas as pd
from utils import Downsample_data, Remove_ouliers, read_data, \
        convert_data, plot_learning_curve
import joblib 
import os
...
def gridsearchCV(X_train, X_test, y_train, y_test, save_path):
    print('-'*80)
    print("Starting Grid Search CV")
    # Logistic Regression 
    print('-'*60)
    print("Grid Search CV for Logistic Regression.")
    log_reg_params = {
        "penalty": ['l2'],
    ...
    name_model = "lgr_gridsearchcv.joblib"
    model_path = os.path.join(save_path,name_model)
    ...
    # KNears
    print('-'*60+'\n'+'-'*60)
    print("Grid Search CV for K Nearest Neighbor.")
    knears_params = {
    ...
    name_model = "knn_gridsearchcv.joblib"
    model_path = os.path.join(save_path,name_model)
    joblib.dump(knears_neighbors, model_path)
    ...
    print('-'*60+'\n'+'-'*60)
    print("Grid Search CV for Support Vector Classifier.")
    svc_params = {
        'C': [0.5, 0.7, 0.9, 1],
    ...
    name_model = "svc_gridsearchcv.joblib"
    model_path = os.path.join(save_path,name_model)
    ...
    print('-'*60+'\n'+'-'*60)
    print("Grid Search CV for Decision Tree Classifier")
    tree_params = {
